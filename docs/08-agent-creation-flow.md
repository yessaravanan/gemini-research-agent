...
Evidence: [main.py](../main.py#L13-L14), [agent.py](../agent.py#L138-L192).
...
Evidence: [agent.py](../agent.py#L138-L239).
...
- Gemini function declarations and runtime mapping: [agent.py](../agent.py)
...
sequenceDiagram
    ...
    Agent->>Memory: _save_memory()
    Agent-->>Main: final_answer
```

Evidence:

- CLI execution: [main.py](../main.py#L6-L23)
- Agent execution: [agent.py](../agent.py#L194-L239)
- Tool calling: [agent.py](../agent.py#L347-L424)

## 6. How Memory Works

Memory is a Python dictionary initialized in `Agent.__init__()` and persisted as `memory.json`.
...
- Initial memory shape: [agent.py](../agent.py#L148-L158)
- Save calls in run flow: [agent.py](../agent.py#L194-L239)
- Tool call append: [agent.py](../agent.py#L485-L491)
- Rate-limit append: [agent.py](../agent.py#L603-L612)
- JSON persistence: [agent.py](../agent.py#L576-L581)
...
flowchart TD
    ...
    M --> N[Create final answer]
    N --> O[Write report.md]
    O --> P[Write memory.json]
    P --> Q[Return result to CLI]
```

Evidence: [agent.py](../agent.py#L141-L239).
...
- Agent persistence beyond generated run memory.
